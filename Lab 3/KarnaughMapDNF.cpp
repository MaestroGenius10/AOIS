...
std::string KarnaughMapDNF::grayCodeToString(int code, int length) {
    std::string binary = std::bitset<32>(code).to_string();
    return binary.substr(32 - length);
}
KarnaughMapDNF::KarnaughMapDNF(const std::vector<std::map<char, bool>>& combinations, const std::vector<bool>& results)
...
void KarnaughMapDNF::initializeMap(int varCount) {
    if (varCount == 3) {
...
std::vector<int> KarnaughMapDNF::generateGrayCode(int bits) {
...
int KarnaughMapDNF::grayCodeToBinary(int gray) {
    int binary = 0;
    for (; gray; gray >>= 1) {
        binary ^= gray;
    }
...
int KarnaughMapDNF::grayCodeToIndex(int gray, const std::vector<int>& grayCodes) {
...
void KarnaughMapDNF::generateMap() {
    fillMap();
}

void KarnaughMapDNF::fillMap() {
...
void KarnaughMapDNF::findAllRegions() {
    findRegions1x1();
    findRegions1x2();
    findRegions2x1();
    findRegions2x2();
...
void KarnaughMapDNF::addRegion(std::set<std::pair<int, int>> region) {
    uniqueRegions.insert(region);
}

void KarnaughMapDNF::filterAndPrintUniqueRegions() {
    for (const auto& region : uniqueRegions) {
        printRegion(region);
    }
}

void KarnaughMapDNF::findRegions1x1() {
...
void KarnaughMapDNF::findRegions1x2() {
    for (int i = 0; i < map.size(); ++i) {
...
void KarnaughMapDNF::findRegions2x1() {
    for (int i = 0; i < map.size(); ++i) {
...
void KarnaughMapDNF::findRegions2x2() {
    for (int i = 0; i < map.size(); ++i) {
        for (int j = 0; j < map[0].size(); ++j) {
            if (map[i][j] &&
                map[(i + 1) % map.size()][j] &&
...
void KarnaughMapDNF::findRegions1x4() {
    for (int i = 0; i < map.size(); ++i) {
        for (int j = 0; j < map[0].size(); ++j) {
            if (map[i][j] &&
                map[i][(j + 1) % map[0].size()] &&
...
void KarnaughMapDNF::findRegions4x1() {
    for (int i = 0; i < map.size(); ++i) {
        for (int j = 0; j < map[0].size(); ++j) {
            if (map[i][j] &&
                map[(i + 1) % map.size()][j] &&
...
void KarnaughMapDNF::findRegions2x4() {
    for (int i = 0; i < map.size(); ++i) {
        int nextRowIndex = (i + 1) % map.size(); 
        for (int j = 0; j < map[0].size(); ++j) {
            int nextColIndex = (j + 1) % map[0].size();
...
void KarnaughMapDNF::findRegions4x2() {
    for (int i = 0; i < map.size(); ++i) {
        int nextRowIndex = (i + 1) % map.size();
        ...
        for (int j = 0; j < map[0].size(); ++j) {
            int nextColIndex = (j + 1) % map[0].size();
            if (map[i][j] && map[nextRowIndex][j] && map[nextNextRowIndex][j] && map[nextNextNextRowIndex][j] &&
                map[i][nextColIndex] && map[nextRowIndex][nextColIndex] && 
...
void KarnaughMapDNF::printRegion(const std::set<std::pair<int, int>>& region) {
    setlocale(LC_ALL, "Ru");
...
void KarnaughMapDNF::findAndSaveMaxRegions() {
...
void KarnaughMapDNF::printMaxRegions() {
    setlocale(LC_ALL, "RU");
...
void KarnaughMapDNF::markUnincluded1x1Regions() {
...
void KarnaughMapDNF::findMaxRegionsContainingSingleCells() {
...
std::vector<std::string> KarnaughMapDNF::regionToExpression(const std::set<std::pair<int, int>>& region, int varCount) {
    if (region.empty()) return {};
...
void KarnaughMapDNF::analyzeChanges(const std::vector<bool>& changes, int varCount, bool isRow, std::vector<bool>& varChange) {
...
bool KarnaughMapDNF::determineState(const std::pair<int, int>& cell, int varIndex, int varCount) {
...
void KarnaughMapDNF::convertMaxRegionsToExpressions() {
    markUnincluded1x1Regions();
    ...
    std::string finalString = builder.vectorToStringDNF(allExpressions);
